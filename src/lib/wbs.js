...
// Pure functions over a flat task list already scoped to ONE project.
...
export function buildTree(tasks) {
  ...
  function make(task, prefix, depth) {
    ...
    if (children.length === 0) {
      ...
      done = children.reduce((s, c) => s + c.rollup.done, 0)
      total = children.reduce((s, c) => s + c.rollup.total, 0)
      // 親のバーは子のスパンを自動集計（最早開始〜最遅終了）
      span = children.reduce((acc, c) => {
  ...
  return roots.map((t, i) => make(t, String(i + 1), 0))
}
